...
class KageParser
{
	...
	public function topic($tid, $pages = 1, $last = true){
		...
		$first_html = iconv('windows-1251', 'UTF-8//IGNORE', $first_html);
	...
	private function getMsgs($link,$from,$to){
		...
		$msg_pattern = '#<tr>.*?<span class="name"><a name="(?P<pId>\d+).*?<b>(?P<userName>.*?)<\/b><\/span><br \/><span class="postdetails">.*?<span class="postdetails">(?P<postDate>.*?)<span class="gen">.*?<span class="postbody">(?<postBody>.*?)<\/span><span class="postbody">.*?(К началу).*?(<a href="profile\.php\?mode=viewprofile&amp;u=((?P<userId>\d+)(.*?)?)"><img src.*?)?\s+<\/tr>\s+<\/table><\/td>\s+<\/tr>#s';
